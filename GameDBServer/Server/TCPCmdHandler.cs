...
using GameDBServer.Logic.AutoTrain;
using GameDBServer.Logic.GameEvents;
using GameDBServer.Logic.GuildLogic;
using GameDBServer.Logic.KT_ItemManager;
using GameDBServer.Logic.KT_Pet;
using GameDBServer.Logic.KT_Recore;
using GameDBServer.Logic.Market;
...
namespace GameDBServer.Server
{
    ...
    internal enum TCPGameServerCmds
    {
        ...
        CMD_KT_DB_SQUASHCRAB_UPDATE = 61005,
        CMD_KT_DB_SQUASHCRAB_RESTORE,
        CMD_KT_DB_AUTOTRAIN_REWARD_LIST = 62000,
        CMD_KT_DB_AUTOTRAIN_REWARD_ADD,
        CMD_KT_DB_AUTOTRAIN_REWARD_UPDATE,
        CMD_KT_DB_AUTOTRAIN_REWARD_CLEAR,
        ...
        /// </summary>
        CMD_KT_TOPRANKING = 50409,

        /// <summary>
        /// Packet đánh dấu vào DB là thằng này đã bú quà top rồi
        /// </summary>
        CMD_KT_UPDATE_REVICE_STATUS = 50410,

        /// <summary>
        /// Packet lấy ra thông tin bảng xếp hạng
        /// </summary>
        CMD_KT_TOPRANKING_INFO = 50411,

        /// <summary>
        /// Packet nhận thưởng
        /// </summary>
        CMD_KT_TOPRANKING_GETAWARD = 50412,
        CMD_KT_RANKING_CHECKING_TOP1 = 50413,


        /// <summary>
        /// Gói tin sử dụng để update đữ liệu vào game db
        /// </summary>
        CMD_SPR_STALL_UDPATE_DB = 50425,

        /// <summary>
        /// Lấy toàn bộ dữ liệu mua bán từ gamedb
        /// </summary>
        CMD_SPR_STALL_QUERRY = 50426,

        CMD_MARKET_QUERY = 50428,

        CMD_MARKET_QUERY_RETURN = 50429,

        CMD_MARKET_BUY = 50430,


        CMD_KT_TIEN_BLACKMARKET_FIND = 55170, // 0x0000D782
        CMD_KT_TIEN_BLACKMARKET_BUY = 55171, // 0x0000D783
        CMD_KT_TIEN_BLACKMARKET_SELL = 55172, // 0x0000D784
        CMD_KT_TIEN_BLACKMARKET_BID = 55173, // 0x0000D785
        CMD_KT_TIEN_BLACKMARKET_CANCEL = 55175,
        #endregion
        ...
        CMD_KT_UPDATE_SKILLS_MAPPING = 50153,

        CMD_KT_DELETE_SKILLS_MAPPING = 50154,

        CMD_KT_CHANGE_SKILLS_MAPPING = 50155
    ...
    internal class TCPCmdHandler
    {
        ...
        public static TCPProcessCmdResults ProcessCmd(GameServerClient client, DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            // Guard null/bad input để tránh NRE storm
            /*if (client == null || dbMgr == null || pool == null || data == null || count <= 0)
            {
                LogManager.WriteLog(LogTypes.Error,
                    $"[DB_CMD_DROP] nID={nID} clientNull={(client == null)} dbMgrNull={(dbMgr == null)} poolNull={(pool == null)} dataNull={(data == null)} count={count}");
                ...
                switch (nID)
            {
                ...
                case (int)TCPGameServerCmds.CMD_KT_G2C_RECHAGE_V2:
                    {
                        result = UserMoneyMgr.CMT_KT_LOG_RECHAGE_2(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                ...
                case (int)TCPGameServerCmds.CMD_KT_UPDATE_SKILLS_MAPPING:
                    {
                        result = TCPCmdHandler.ProcessUpdateSkillMapping(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_DELETE_SKILLS_MAPPING:
                    {
                        result = TCPCmdHandler.ProcessDeleteSkillMapping(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_CHANGE_SKILLS_MAPPING:
                    {
                        result = TCPCmdHandler.ProcessChangeSkillMapping(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_MARKET_QUERY:
                    {
                        result = ProcessMarketQuery(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                ...
                case (int)TCPGameServerCmds.CMD_KT_DB_SQUASHCRAB_UPDATE:
                    {
                        result = GameEventManager.Instance.ProcessGameeventSys(pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_DB_SQUASHCRAB_RESTORE:
                    {
                        result = GameEventManager.Instance.ProcessGameeventLoad(pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                #endregion
                /*#region Auto train
                ...
                case (int)TCPGameServerCmds.CMD_KT_DB_AUTOTRAIN_REWARD_ADD:
                    {
                        try
                        {
                            result = AutoTrain.Instance.ProcessInsert(pool, nID, data, count, out tcpOutPacket);
                        }
                        catch (Exception ex)
                        {
                            LogManager.WriteLog(LogTypes.Error,$"[AUTOTRAIN_ADD_CRASH] CmdID={nID}, DataSize={count}, Exception={ex}");
                            result = TCPProcessCmdResults.RESULT_FAILED;
                        }
                    }
                    break;

                case (int)TCPGameServerCmds.CMD_KT_DB_AUTOTRAIN_REWARD_UPDATE:
                    {
                        try
                        {
                            result = AutoTrain.Instance.ProcessRewardSys(pool, nID, data, count, out tcpOutPacket);
                        }
                        catch (Exception ex)
                        {
                            LogManager.WriteLog(LogTypes.Error,$"[AUTOTRAIN_UPDATE_CRASH] CmdID={nID}, DataSize={count}, Exception={ex}");
                            result = TCPProcessCmdResults.RESULT_FAILED;
                        }
                    }
                    break;

                case (int)TCPGameServerCmds.CMD_KT_DB_AUTOTRAIN_REWARD_CLEAR:
                    {
                        try
                        {
                            result = AutoTrain.Instance.ProcessRewardDelete(pool, nID, data, count, out tcpOutPacket);
                        }
                        catch (Exception ex)
                        {
                            LogManager.WriteLog(LogTypes.Error,$"[AUTOTRAIN_CLEAR_CRASH] CmdID={nID}, DataSize={count}, Exception={ex}");
                            result = TCPProcessCmdResults.RESULT_FAILED;
                        }
                    }
                    break;

                case (int)TCPGameServerCmds.CMD_KT_DB_AUTOTRAIN_REWARD_LIST:
                    {
                        try
                        {
                            result = AutoTrain.Instance.ProcessRewardLoad(pool, nID, data, count, out tcpOutPacket);
                        }
                        catch (Exception ex)
                        {
                            LogManager.WriteLog(LogTypes.Error,$"[AUTOTRAIN_LIST_CRASH] CmdID={nID}, DataSize={count}, Exception={ex}");
                            result = TCPProcessCmdResults.RESULT_FAILED;
                        }
                    }
                    break;
                ...
                #region Chợ đen
                case (int)TCPGameServerCmds.CMD_KT_TIEN_BLACKMARKET_FIND:
                    {
                        result = BlackMarket.CMD_KT_TIEN_BLACKMARKET_FIND(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_TIEN_BLACKMARKET_BUY:
                    {
                        result = BlackMarket.CMD_KT_TIEN_BLACKMARKET_BUY(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_TIEN_BLACKMARKET_SELL:
                    {
                        result = BlackMarket.CMD_KT_TIEN_BLACKMARKET_SELL(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_TIEN_BLACKMARKET_BID:
                    {
                        result = BlackMarket.CMD_KT_TIEN_BLACKMARKET_BID(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                case (int)TCPGameServerCmds.CMD_KT_TIEN_BLACKMARKET_CANCEL:
                    {
                        result = BlackMarket.CMD_KT_TIEN_BLACKMARKET_CANCEL(dbMgr, pool, nID, data, count, out tcpOutPacket);
                        break;
                    }
                #endregion 
                default:
            ...
            {
                LogManager.WriteLog(LogTypes.Error,
                    $"[DB_CMD_CRASH] nID={nID} ({((TCPGameServerCmds)nID).ToString()}) bytes={count} costMs={sw.ElapsedMilliseconds} ex={ex}");
                return TCPProcessCmdResults.RESULT_FAILED;
            }
            finally
            {
                ...
                // Log slow cmd (ngưỡng tuỳ bạn: 200ms/500ms/1000ms)
                if (sw.ElapsedMilliseconds >= 2000)
                {
                    LogManager.WriteLog(LogTypes.Warning,
                        $"[DB_CMD_SLOW] nID={nID} ({((TCPGameServerCmds)nID).ToString()}) bytes={count} costMs={sw.ElapsedMilliseconds}");
                }
            }
  }

        public static TCPProcessCmdResults ProcessMarketQuery(
            DBManager dbMgr,
            ...
            out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                // CMD này không cần param, nhưng vẫn đọc cho đúng format
                cmdData = new UTF8Encoding().GetString(data, 0, count);
            }
            catch (Exception)
            {
                LogManager.WriteLog(LogTypes.Error,
                    string.Format("Wrong socket data CMD={0}", (TCPGameServerCmds)nID));

                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(
                    pool,
                    "0",
            ...
            {
                // ===== LOAD MARKET DATA =====
                List<MarketItemData> list = MarketManager.LoadAllMarketItems();

                tcpOutPacket = DataHelper.ObjectToTCPOutPacket(
                    list,
                    pool,
                    nID
                );
        ...
        private static TCPProcessCmdResults ProcessCreateRoleCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    strcmd = string.Format("{0}:{1}", 1, string.Format("{0}${1}${2}${3}${4}${5}${6}", roleID, sex, factionID, rolename, nInitLevel, 0,
                        "-1_-1_-1_0_-1"));
                }
        ...
        private static TCPProcessCmdResults ProcessAddGoodsCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (fields.Length != 13)
                {
                    LogManager.WriteLog(LogTypes.Error, string.Format("Error Socket params count not fit ProcessAddGoodsCmd CMD={0}, Recv={1}, CmdData={2}", (TCPGameServerCmds)nID, fields.Length, cmdData));
                    tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, "0", (int)TCPGameServerCmds.CMD_DB_ERR_RETURN);
                    return TCPProcessCmdResults.RESULT_DATA;
                }
        ...
        private static TCPProcessCmdResults ProcessAddSkillCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int factionId = Convert.ToInt32(fields[2]);
                ...
                int ret = DBWriter.AddSkill(dbMgr, roleID, skillID, factionId);
                if (ret > 0)
                {
        ...
        private static TCPProcessCmdResults ProcessDelSkillCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                bool ret = DBWriter.DeleteSkill(dbMgr, roleID, skillID, factionId);
                if (ret)
                {
                    if (null == dbRoleInfo.SkillDataList)
                    {
        ...
        private static TCPProcessCmdResults ProcessUpSkillInfoCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int factionId = Convert.ToInt32(fields[6]);
                ...
                bool ret = DBWriter.UpdateSkillInfo(dbMgr, roleID, skillID, skillLevel, lastUsedTick, cooldownTick, exp, factionId);
        ...
        private static TCPProcessCmdResults ProcessGetPaiHangListCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                List<PlayerRanking> RankData = RankingManager.getInstance().GetRankOld(_RankMode, roleID, pagennumber);
        ...
        private static TCPProcessCmdResults ProcessSendUserMailCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    LogManager.WriteLog(LogTypes.Error,
                        $"Error Socket params count not fit CMD={(TCPGameServerCmds)nID}, Recv={fields.Length}, CmdData={cmdData}");
                ...
                string title = Encoding.UTF8.GetString(Convert.FromBase64String(fields[4]));
                string content = Encoding.UTF8.GetString(Convert.FromBase64String(fields[5]));

                // Gán lại để AddMail dùng
                fields[4] = title;
                fields[5] = content;
                /// Nếu độ dài 10 nghĩa là giá trị cuối quy định phải kiểm tra người nhận tồn tại không
                if (fields.Length == 12)
                {
                    int.TryParse(fields[11], out int checkReceiverExist);
        ...
        private static TCPProcessCmdResults ProcessUpdateRoleParamCmd(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (name == "AutoSettings")
                {
                    int fractionId = Convert.ToInt32(fields[3]);
                    int subId = Convert.ToInt32(fields[4]);
                    string mainQuickKeys = fields[5];
                    string otherQuickKeys = fields[6];
                    Global.UpdateSkillMappings(dbRoleInfo, value, fractionId, subId, mainQuickKeys, otherQuickKeys);
                }

                if (name == "PropStrength")
                {
                    int fractionId = Convert.ToInt32(fields[3]);
                    int subId = Convert.ToInt32(fields[4]);
                    string mainQuickKeys = fields[5];
                    string otherQuickKeys = fields[6];
                    Global.UpdateSkillProps(dbRoleInfo, value, fractionId, subId, mainQuickKeys, otherQuickKeys, "prop_strength");
                    Global.ReloadDBRoleInfo(dbRoleInfo);
                }

                if (name == "PropIntelligence")
                {
                    int fractionId = Convert.ToInt32(fields[3]);
                    int subId = Convert.ToInt32(fields[4]);
                    string mainQuickKeys = fields[5];
                    string otherQuickKeys = fields[6];
                    Global.UpdateSkillProps(dbRoleInfo, value, fractionId, subId, mainQuickKeys, otherQuickKeys, "prop_intelligence");
                    Global.ReloadDBRoleInfo(dbRoleInfo);
                }

                if (name == "PropDexterity")
                {
                    int fractionId = Convert.ToInt32(fields[3]);
                    int subId = Convert.ToInt32(fields[4]);
                    string mainQuickKeys = fields[5];
                    string otherQuickKeys = fields[6];
                    Global.UpdateSkillProps(dbRoleInfo, value, fractionId, subId, mainQuickKeys, otherQuickKeys, "prop_dexterity");
                    Global.ReloadDBRoleInfo(dbRoleInfo);
                }

                if (name == "PropConstitution")
                {
                    int fractionId = Convert.ToInt32(fields[3]);
                    int subId = Convert.ToInt32(fields[4]);
                    string mainQuickKeys = fields[5];
                    string otherQuickKeys = fields[6];
                    Global.UpdateSkillProps(dbRoleInfo, value, fractionId, subId, mainQuickKeys, otherQuickKeys, "prop_constitution");
                    Global.ReloadDBRoleInfo(dbRoleInfo);
                }
        ...
        /// <summary>
        /// Thực hiện update t_skill_mapping
        /// </summary>
        /// <param name="dbMgr"></param>
        /// <param name="pool"></param>
        /// <param name="nID"></param>
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        private static TCPProcessCmdResults ProcessUpdateSkillMapping(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int fractionId = Convert.ToInt32(fields[2]);
                int subId = Convert.ToInt32(fields[3]);
                string mainQuickKeys = fields[4];
                string otherQuickKeys = fields[5];
                Global.UpdateSkillMappings(dbRoleInfo, value, fractionId, subId, mainQuickKeys, otherQuickKeys);
        ...
        /// <summary>
        /// Thực hiện delete t_skill_mapping
        /// </summary>
        /// <param name="dbMgr"></param>
        /// <param name="pool"></param>
        /// <param name="nID"></param>
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        private static TCPProcessCmdResults ProcessDeleteSkillMapping(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int fractionId = Convert.ToInt32(fields[1]);
                Global.DeleteSkillMappings(dbRoleInfo, fractionId);
        ...
        /// <summary>
        /// Thực hiện change t_skill_mapping
        /// </summary>
        /// <param name="dbMgr"></param>
        /// <param name="pool"></param>
        /// <param name="nID"></param>
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        private static TCPProcessCmdResults ProcessChangeSkillMapping(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int fractionId = Convert.ToInt32(fields[1]);
                Global.ChangeSkillMapping(dbRoleInfo, fractionId);
