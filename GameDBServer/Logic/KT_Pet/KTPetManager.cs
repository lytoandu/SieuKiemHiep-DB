...
namespace GameDBServer.Logic.KT_Pet
{
    ...
    public class KTPetManager
    {
        ...
        private bool DB_UpdatePetData_Attributes(int petID, long str, long dex, long sta, long intel, long remainPoints)
        {
            MySQLConnection conn = null;
        ...
        /// <param name="petID"></param>
        /// <param name="hp"></param>
        private bool DB_UpdatePetData_HP(int petID, double hp)
        {
            MySQLConnection conn = null;
        ...
        /// <param name="level"></param>
        /// <param name="exp"></param>
        private bool DB_UpdatePetData_BeforeQuitGame(int petID, double hp, int joyful, int life, int level, int exp)
        {
            MySQLConnection conn = null;
        ...
        public TCPProcessCmdResults ProcessPetUpdateAttributes(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int petID = int.Parse(fields[1]);
                /// Sức mạnh
                long str = long.Parse(fields[2]);
                /// Thân pháp
                long dex = long.Parse(fields[3]);
                /// Ngoại công
                long sta = long.Parse(fields[4]);
                /// Nội công
                long intel = long.Parse(fields[5]);
                /// Tiềm năng
                long remainPoints = long.Parse(fields[6]);
        ...
        public TCPProcessCmdResults ProcessPetUpdateHP(DBManager dbMgr, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int petID = int.Parse(fields[1]);
                /// Sinh lực pet
                double hp = double.Parse(fields[2]);
