    ...
    internal class Global
    {
        ...
        public static void DBRoleInfo2RoleDataEx(DBRoleInfo dbRoleInfo, RoleDataEx roleDataEx)
        {
            ...
            lock (dbRoleInfo)
            {
                ...
                roleDataEx.OccupationDataList = dbRoleInfo.OccupationDataList;
        ...
        /// <param name="mailData"></param>
        /// <returns></returns>
        public static int AddMailold(DBManager dbMgr, string[] fields, out int addGoodsCount)
        {
            ...
            if(boundToken > 0)
            {
                ...
                //添加邮件实体
                mailID = DBWriter.AddMailBody(dbMgr, senderrid, senderrname, receiverrid, reveiverrname, subject, content, hasFetchAttachment, 0, boundToken,0,0);
                
            }
            else
            {
                
                mailID = DBWriter.AddMailBody(dbMgr, senderrid, senderrname, receiverrid, reveiverrname, subject, content, hasFetchAttachment, boundMoney, 0,0,0);
        ...
        {
            int result1;
            int.TryParse(fields[0], out result1);
            string str1 = DataHelper.Base64Encode(fields[1]);
            int result2;
            int.TryParse(fields[2], out result2);
            string maxLevelRoleName = DataHelper.Base64Encode(fields[3]);
            string str2 = DataHelper.Base64Encode(fields[4]);
            string str3 = DataHelper.Base64Encode(fields[5]);
            int result3;
            int.TryParse(fields[6], out result3);
            int result4;
            int.TryParse(fields[7], out result4);
            string field = fields[8];
            int result5;
            int.TryParse(fields[9], out result5);
            int result6;
            int.TryParse(fields[10], out result6);
            if (maxLevelRoleName == "")
            {
                string userID = "";
                Global.GetRoleNameAndUserID(dbMgr, result2, out maxLevelRoleName, out userID);
            }
            string senderrname = str1.Replace('$', ':');
            string reveiverrname = maxLevelRoleName.Replace('$', ':');
            string subject = str2.Replace('$', ':');
            string content = str3.Replace('$', ':');
            addGoodsCount = 0;
            DBRoleInfo dbRoleInfo = dbMgr.GetDBRoleInfo(result1);
            if (dbRoleInfo != null)
                senderrname = Global.FormatRoleName(dbRoleInfo);
            int hasFetchAttachment = !string.IsNullOrEmpty(field) || result3 > 0 || result4 > 0 || result5 > 0 || result6 > 0 ? 1 : 0;
            int num = DBWriter.AddMailBody(dbMgr, result1, senderrname, result2, reveiverrname, subject, content, hasFetchAttachment, result3, result4, result5, result6);
            if (num >= 0)
            {
                addGoodsCount = Global.AddMailGoods(dbMgr, num, field.Split('|'));
                DBWriter.UpdateLastScanMailID(dbMgr, result2, num);
            }
            return num;
        }
        /// <summary>
        ...
        /// <param name="goodsID"></param>
        /// <returns></returns>
        public static void UpdateSkillMappings(DBRoleInfo dbRoleInfo, string value, int fractionId,
            int subId, string mainQuickKeys, string otherQuickKeys)
        {
            DBWriter.UpdateSkillMapping(dbRoleInfo.RoleID, value, fractionId, subId, mainQuickKeys,
                otherQuickKeys);

            ReloadDBRoleInfo(dbRoleInfo);
        }

        public static void UpdateSkillProps(DBRoleInfo dbRoleInfo, string value, int fractionId,
         int subId, string mainQuickKeys, string otherQuickKeys, string columnName)
        {
            DBWriter.UpdateSkillProps(dbRoleInfo.RoleID, value, fractionId, subId, mainQuickKeys,
                otherQuickKeys, columnName);
        }
        ...
        /// <param name="goodsID"></param>
        /// <returns></returns>
        public static void DeleteSkillMappings(DBRoleInfo dbRoleInfo, int fractionId)
        {
            DBWriter.DeleteSkillMapping(dbRoleInfo.RoleID, fractionId);
        }

        public static void ChangeSkillMapping(DBRoleInfo dbRoleInfo, int fractionId)
        {
            DBWriter.ChangeSkillMapping(dbRoleInfo.RoleID, fractionId);

            DBManager.getInstance().ReloadDBRoleInfo(dbRoleInfo);
        }

        public static void ReloadDBRoleInfo(DBRoleInfo dbRoleInfo)
        {
            DBManager.getInstance().ReloadDBRoleInfo(dbRoleInfo);
        }
    }
