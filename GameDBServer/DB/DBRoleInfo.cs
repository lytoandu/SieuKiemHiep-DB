    ...
    public class DBRoleInfo
    {
        ...
        */
            ...
            if (cmd.Table.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < cmd.Table.Rows.Count; i++)
                {
                    SkillData skillData = new SkillData()
                    {
                        ...
                        Occupation = Convert.ToInt32(cmd.Table.Rows[i]["occupation"].ToString()),
                    };
        ...
        public bool Query(MySQLConnection conn, int roleID, bool bUseIsdel = true)
        {
            ...
            //查询经脉的列表信息
            cmd = new MySQLSelectCommand(conn,
                     new string[] { "Id", "skillid", "skilllevel", "lastusedtick", "cooldowntick", "exp", "occupation" },
            ...
            cmd = new MySQLSelectCommand(conn,
                new string[] { "occupation", "primary_flag", "sub_id", "main_quick_keys", "other_quick_keys", "auto_settings"
                , "prop_strength", "prop_intelligence", "prop_dexterity", "prop_constitution"
                },
                new string[] { "t_skills_mapping" }, new object[,] { { "rid", "=", roleID } }, null, null);
            DBRoleInfo.DBTableRow2RoleInfo_Occupations(this, cmd);
        ...
        /// Danh sách phái
        /// </summary>
        public List<OccupationData> OccupationDataList
        {
            get;
            set;
        }
        ...
        private static void DBTableRow2RoleInfo_Occupations(DBRoleInfo dbRoleInfo, MySQLSelectCommand cmd)
        {
            if (cmd.Table.Rows.Count <= 0) return;

            dbRoleInfo.OccupationDataList = new List<OccupationData>();
            for (int i = 0; i < cmd.Table.Rows.Count; i++)
            {
                var data = new OccupationData()
                {
                    Id = Convert.ToInt32(cmd.Table.Rows[i]["occupation"].ToString()),
                    SubId = Convert.ToInt32(cmd.Table.Rows[i]["sub_id"].ToString()),
                    PrimaryFlag = Convert.ToInt32(cmd.Table.Rows[i]["primary_flag"].ToString()),
                    MainQuickBarKeys = cmd.Table.Rows[i]["main_quick_keys"].ToString(),
                    OtherQuickBarKeys = cmd.Table.Rows[i]["other_quick_keys"].ToString(),
                    AutoSettings = cmd.Table.Rows[i]["auto_settings"].ToString(),
                    Strength = Global.SafeConvertToInt32(cmd.Table.Rows[i]["prop_strength"].ToString()),
                    Intelligence = Global.SafeConvertToInt32(cmd.Table.Rows[i]["prop_intelligence"].ToString()),
                    Dexterity = Global.SafeConvertToInt32(cmd.Table.Rows[i]["prop_dexterity"].ToString()),
                    Constitution = Global.SafeConvertToInt32(cmd.Table.Rows[i]["prop_constitution"].ToString()),
                };
                dbRoleInfo.OccupationDataList.Add(data);

                Console.WriteLine(string.Format("{0} {1} {2} {3} {4}",
                    dbRoleInfo.RoleID, data.Strength, data.Intelligence, data.Dexterity, data.Constitution));
                if(data.PrimaryFlag == 1)
                {
                    dbRoleInfo.Occupation = data.Id;
                    dbRoleInfo.SubID = data.SubId;
                }
            }
        }

        public void ReloadRow2RoleInfoc_Occupations(MySQLConnection conn, DBRoleInfo dbRoleInfo)
        {
            MySQLSelectCommand cmd = new MySQLSelectCommand(conn,
                new string[] { "occupation", "primary_flag", "sub_id", "main_quick_keys", "other_quick_keys", "auto_settings"
                , "prop_strength", "prop_intelligence", "prop_dexterity", "prop_constitution"
                },
            new string[] { "t_skills_mapping" }, new object[,] { { "rid", "=", dbRoleInfo.RoleID } }, null, null);
            DBTableRow2RoleInfo_Occupations(dbRoleInfo, cmd);
        }
